...
| 动作脚本与固定工况输入 | L2 | `docs/scripting.md`, `scripts_lib/*.yaml`, `scripts/smoke_test.py`, `frontend/tests/e2e/workflow-smoke.spec.ts` 的脚本库 workflow | 已覆盖内置脚本库载入、启动、停止，以及脚本声明的参考路径/桩布局铺设；仍属于仿真内固定输入，不等价于遥控器/实物平台闭环验证。 |
...
2. 浏览器端自动化仍偏 smoke：关键 workflow 页截图、车辆几何拖拽、分析页截图/图表增删/hover cursor/drag-to-zoom/PNG 导出、负载页图表绘制/原理说明弹窗、场景页标准路径生成/跟踪/清除、路面扰动编辑新建/编辑/清空、故障注入添加/启停/清空、动作脚本库载入/启动/停止、run 数据读取失败异常态、试验页 batch 启动失败异常态、车辆页参数应用被拒绝异常态、回放时间轴、命令面板导航和手柄配置编辑已有覆盖，但更多深层页面状态和更少见后端失败分支仍主要靠人工端到端验证。
