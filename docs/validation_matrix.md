...
3. 接入真实 reference 数据：checker 已能汇总误差表、reviewer notes 和独立来源 artifact checksum；下一步是放入真实外部/实测 benchmark，并由人工审查误差解释。
