...
| 动作脚本与固定工况输入 | L2 | `docs/scripting.md`, `scripts_lib/*.yaml`, `scripts/smoke_test.py`, `frontend/tests/e2e/workflow-smoke.spec.ts` 的脚本库 workflow | 已覆盖内置脚本库载入、启动、停止、脚本声明的参考路径/桩布局铺设，以及 YAML 解析失败异常态；仍属于仿真内固定输入，不等价于遥控器/实物平台闭环验证。 |
