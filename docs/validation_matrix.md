...
| 参考/外部/实测对照接入 | L3 | `docs/reference_benchmark_protocol.md`, `validation_data/README.md`, `validation_data/analytic_steady_circle_30kmh/`, `validation_data/analytic_step_steer_30kmh/`, `scripts/check_reference_benchmarks.py`, `scripts/scaffold_reference_benchmark.py`, `scripts/normalize_reference_csv.py`, `scripts/promote_reference_benchmark.py`, `backend/tests/test_reference_benchmarks.py`, `backend/tests/test_normalize_reference_csv.py`, `backend/tests/test_promote_reference_benchmark.py` | 已有两个解析 benchmark，`--require-data` 可通过；独立来源接入已有 incoming 脚手架、原始 CSV 归一、时间窗裁剪/归零、provenance 必填校验、source artifact SHA-256 校验和 promotion 门禁；`--require-independent-source` 会继续失败直到接入通过检查的外部工具、台架、缩比车或实车 benchmark，因此当前不能提升到外部工具对照或 L4 实测等级。 |
