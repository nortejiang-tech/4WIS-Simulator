...
| run 回放与分析页 | L2 | `frontend/tests/e2e/workflow-smoke.spec.ts`, `frontend/src/components/ReplayPanel.tsx`, `frontend/src/charts/uplotFactory.ts` | 已有浏览器 smoke 覆盖实验到分析页链路、通道增删/加图、hover cursor/drag-to-zoom/双击复位、PNG 导出、工作区与关键 workflow 页截图 attachment、数据录制开始/停止/CSV 导出、回放时间轴 scrub、回放 meta 读取失败默认尺寸 fallback、负载页图表绘制/原理说明弹窗、原理页 demo 计算失败、场景页标准路径生成/跟踪/清除、路面扰动编辑新建/编辑/清空、故障注入添加/启停/清空，以及分析页 run 列表读取失败、run 数据读取失败、试验页实验库读取失败、试验页机动模板读取失败、试验页 batch 启动失败、运行页模型/路面控制失败、Python 策略状态读取失败、车辆页参数应用被拒绝、车辆页项目列表读取失败、车辆页项目 YAML 加载失败、场景列表读取失败、场景加载失败、场景清除失败、路径模板读取失败、路径版本刷新失败、场景版本刷新失败、扰动更新/删除/清空失败、故障列表读取失败、故障添加/切换/删除/清空失败、脚本库读取失败、脚本状态读取失败、负载页扫图计算失败、负载页车型库读取失败、负载页参数读取失败、负载页车型载入/保存/应用失败、负载页敏感度扫描失败、命令面板模型切换失败、录制 CSV 导出失败异常态；仍需扩展更多深层页面状态和更少见后端失败分支截图。 |
...
| 动作脚本与固定工况输入 | L2 | `docs/scripting.md`, `scripts_lib/*.yaml`, `scripts/smoke_test.py`, `frontend/tests/e2e/workflow-smoke.spec.ts` 的脚本库 workflow | 已覆盖内置脚本库载入、启动、停止、脚本声明的参考路径/桩布局铺设、路径模板读取失败，以及脚本库读取失败和 YAML 解析失败异常态；仍属于仿真内固定输入，不等价于遥控器/实物平台闭环验证。 |
...
2. 浏览器端自动化仍偏 smoke：关键 workflow 页截图、车辆几何拖拽、分析页截图/图表增删/hover cursor/drag-to-zoom/PNG 导出、负载页图表绘制/原理说明弹窗、原理页 demo 计算失败异常态、场景页标准路径生成/跟踪/清除、路面扰动编辑新建/编辑/清空、故障注入添加/启停/清空、动作脚本库载入/启动/停止、数据录制开始/停止/CSV 导出、回放 meta 读取失败异常态、路径模板读取失败异常态、路径版本刷新失败异常态、场景版本刷新失败异常态、扰动和故障写操作失败异常态、脚本库读取失败异常态、脚本状态读取失败异常态、脚本解析失败异常态、run 列表读取失败异常态、run 数据读取失败异常态、试验页实验库读取失败异常态、试验页机动模板读取失败异常态、试验页 batch 启动失败异常态、运行页模型/路面控制失败异常态、Python 策略状态读取失败异常态、车辆页参数应用被拒绝异常态、车辆页项目列表读取失败异常态、车辆页项目 YAML 加载失败异常态、场景列表/加载/清除失败异常态、负载页扫图计算失败异常态、负载页车型库读取失败异常态、负载页参数读取失败异常态、负载页车型载入/保存/应用失败异常态、负载页敏感度扫描失败异常态、命令面板模型切换失败异常态、录制 CSV 导出失败异常态、回放时间轴、命令面板导航和手柄配置编辑已有覆盖，但更多深层页面状态和更少见后端失败分支仍主要靠人工端到端验证。
