...
- **状态**：**已冻结为开发基准并全部落地**。M1（骨架/控制器协议/执行器/传感器/逐轮通道）→ M2（控制库 v1 + 前馈三件套 + 阶跃评估）→ M3（调参工作台，110% 验收）→ M4（进阶库 dob/adrc/smc/mpc/h_inf + Simulink 接口框架）均已实现并测试钉住；使用手册见 [`steering_control_guide.md`](steering_control_guide.md)
