...
1. **执行器模型细度**：是否建模电机电流环（三环：位置/速度/电流）？还是电流环用一阶近似、控制层只到"力矩指令"？→ **建议**：先两环半（位置+速度环，电流环一阶近似），包络沿用 `motor.py`——三环细节留给 Simulink 接口验证时再对齐。若你评估重点包含"电流环带宽对角度跟踪的影响"，则需三环。
2. **逐轮执行器改造范围**：FR-5 会删除前轴左右平均、给后轴建执行器——这会动 `steering_link`/`dynamic`/`multibody` 的核心路径，并可能改变"有 plant 时前轴行为"的数值（现在左右平均是个简化）。**确认**：本改造在 M1 范围内吗？（建议：是——否则"4WIS 逐轮评估"无从谈起。）
3. **Simulink 集成优先级**：FMI 共仿真 vs 代码生成？→ **建议**：先 FMI（不改你的模型结构、保留 Simulink 生态），代码生成作为低延迟备选。需要确认你的 Simulink 版本与是否有 FMI Toolbox/Embedded Coder 授权。
4. **手轮回路是否纳入**：SBW 的手感/反馈作动器是否也要"控制策略"（它本质是力闭环）？→ **建议**：本次只做角度跟随，手感合成留现有实现；但接口上预留"反馈作动器也走控制器协议"的可能。
5. **自动调参的验收标准**：如何定义"调好了"？→ **建议**：工况集上加权指标（如 ITAE + 控制量峰值约束）不劣于人工解析整定的 110%，且调参过程全自动可复现。这个门槛是否需要更严格（如 ≤105%）？
6. **评估工况集**：阶跃/正弦/扰动的幅值频点是否就用 ISO 13674 中心区 + 停车 scrub + 高速避让三类？（建议：是，与现有 procedures 对齐。）
7. **控制层的"目标角"粒度**：目标角是否带前馈的角速度/角加速度（来自策略层或轨迹规划）？现有策略层只给角度。→ **建议**：第一版由控制层内部对目标角求导（带滤波）产生前馈量；接口预留显式 rate/accel 字段，将来策略层可供给。
