...
- 已完成：reference checker 新增 `--check-report`，pre-release 会用它校验已提交的 `docs/reports/reference_benchmark_review.md` 是否与当前 benchmark 数据一致，避免审查材料静默过期。
...
- 已完成：新增 `scripts/scaffold_reference_benchmark.py`，用于把 CarSim/CarMaker、台架、缩比车或实车 benchmark 先生成到 `validation_data/.incoming/`；模板不会被正式 checker 当作证据，补齐真实数据后再移动到正式 `validation_data/<benchmark_id>/`。
...
- 已完成：Playwright 覆盖试验页实验库保存和删除失败异常态，断言编辑器输入不丢、已保存实验仍留在库中，并生成异常态 screenshot attachment。
...
- 已完成：Playwright 覆盖场景页路面扰动更新、删除和清空失败异常态，断言已有扰动不被误删、放置入口恢复可用，并生成异常态 screenshot attachment。
- 已完成：Playwright 覆盖 2D 画布扰动放置失败和选中扰动拖动失败异常态，断言错误 toast 可见、扰动选择状态仍可用，并生成异常态 screenshot attachment。
...
- 已完成：Playwright 覆盖场景页故障添加、停用/启用、删除和清空失败异常态，断言已有故障不被误删、按钮恢复可用，并生成异常态 screenshot attachment。
...
- 已完成：Playwright 覆盖运行页数据录制状态读取、开始录制、停止录制和 CSV 导出失败异常态，断言错误停留在录制面板、控制入口恢复可用，并生成异常态 screenshot attachment。
...
- 已完成：单轮失效报告中剩余的静态机构、FMEA、HARA、C 分级和工具演进表格已迁移到 `html_table`/`html_cell`，并补充 raw/escaped cell 混用单测；重跑报告确认 metrics JSON 和图片无变化。
...
- 已完成：Panel 内容共享控件（图表块、策略按钮、读数、按钮行、参数列表等）从全局 `styles.css` 拆到 `PanelContent.css`；全局 `styles.css` 只剩主题 token、字体/字号 token 和 reset。
- 已完成：全局 `mono`/`small` 工具类已迁移为 App、HUD、Workflow 和 PanelContent 的语义类，并改用 `--font-mono` / `--font-size-xs` token；当前入口 `index.css` 为 15.81 kB / gzip 3.47 kB。
- 下一步：后续新增样式默认放在页面/组件私有 CSS，避免重新引入根级工具类。
