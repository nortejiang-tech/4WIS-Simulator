...
## 0.104.0 — 2026-09-09（物理数学修订 · 三种交互路径 · Agent 标准接口 · 双平台便携交付）

详见 [`docs/v0.104.0_changelog.md`](docs/v0.104.0_changelog.md)。

### 修复 (Fixed)

- **底盘动力学/几何 P0 纠偏**：质心与轴距中点力矩基准统一、轮胎接触/载荷守恒边界、主销—齿条虚功链、
  RK4 状态副作用、轮速积分、reset/录制生命周期、FL/FR/RL/RR 显示和坐标语义统一。黄金基线仅在独立平衡、
  虚功、非线性与步长对照后迁移，阈值未放宽。
- **交互可靠性**：人工输入的焦点/断连/超时释放，脚本动作生命周期，Agent 请求幂等、revision 冲突、数值失败
  前缀与导出语义，以及窄屏工作台布局。

### 新增 (Added)

- **三种交互与全量输出**：手动驾驶、YAML 脚本工况、隔离 Agent 固定步长会话；统一 run 支持图形、全量
  CSV/JSON/ZIP、SVG 和 manifest/SHA-256。
- **便携 MCP 交付**：macOS/Windows zip 内含 `sim4wis_mcp`、SDK、`agent_mcp` 启动器和
  `--print-config`；新增 `scripts/check_portable_package.py` 对源码、SDK、数据、手册、CRC 和原生启动路径做门禁。
- **当前文档集**：三种交互指南、隔离会话 ADR、物理/交互评审和 EVO-Coder 调用报告随 release 交付。

## 0.103.0 — 2026-08-16（转向控制层主线收尾 · 柔度三臂 · 调参工作台五通道 · 超包线定量研究 · 规格/选型闭环互证 · 共置反馈/刚度/抗间隙三项研究）

详见 [`docs/v0.103.0_changelog.md`](docs/v0.103.0_changelog.md)。
...
- **柔度车辆级复调 + 评估维度（方向 3 收尾 ②）**：角级增益不迁移到车辆环——
  ...
  3/3 判据绿）。**间隙量化（收尾 ③）**：柔度环容忍 ≤5 mrad（超调 0.1→3.0%、
  中心区极限环 0.0007 rad），10 mrad 退化（35%、狩猎 0.012 rad），20 mrad
  越限（192%、峰值偏差 0.051 rad）——精密齿轮箱可接受，更大间隙需抗间隙
...
- **电机侧传感器通道研究（共置反馈，实测负结果）**：
  `scripts/devtools/motor_channel_study.py` 对三种共置结构（电机速率阻尼 /
  电机角位置反馈 / 共置级联）网格钉住——轮位置环在任何结构下都跨传动弹簧
  闭环，外环带宽被反共振钉在 ~2–3 Hz（k=1200），现有轮侧设计已贴近天花板；
  共置结构最多 +10–20% 角级带宽且间隙下更差（电机角反馈看不见间隙：轮侧
...
- **抗间隙结构研究（轮侧手段实测证伪）**：
  ...
  kp 减半只把超调换成静差、kd 加大激发共振。**轮侧无解**；杠杆：更紧硬件间隙
  （2–5 mrad 精密齿轮箱）> 电机通道过隙速率控制 > 更软 k。不落控制层死代码。
...
- **阶跃/扫频 procedure 扩展到 9 控制器（§6.5）**：
  ...
  smc/mpc/h_inf 无低通前馈——教训 #6），与扰动臂对齐，全库对比一条命令。
