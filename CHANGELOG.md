...
- **负载扰动评估协议（FR-10 扰动臂，三臂齐备）**：
  ...
  回正力阶跃（实测 1.1–2.1 kN）即角执行器负载扰动；分析器从齿条力
  自身检测扰动时刻（滑窗最大阶跃、中点计时、绝对地板 100 N），量峰值
  偏差与回带时间（尾样本在带外 = 如实报"未恢复"）。首份数据：恢复时间
  反映结构——adrc 0.045 s / smc 0.060 s（观测器/滑移型）vs pid 0.145 s
  （积分器型，2–3 倍慢）。全员速度前馈（负载补偿是被比较的对象）。
  该工况暴露 **SMC 第二次滑动可达性违反**：无负载前馈时冰面持续负载
  ≈40 N·m ≫ k=25，轮子漂出 0.129 rad、4.4 s 才爬回——切换增益
  提至 60 N·m（覆盖 ~3 kN 齿条力）。

- **扫频评估协议（FR-10 频域臂）**：`procedures/tracking_sweep_response.yaml`
  + `trk_*` 频域指标族（0.5/1/2/5/10 Hz 幅值比、2 Hz 相位滞后、−3 dB 带宽）。
  分析器从命令信号自身分段（插值过零半周期），逐段相关最小二乘拟合，
  对数插值带宽；带宽超出扫掠上界时**只报下界并拒绝给数**（不伪装）。
  首份数据：open_loop 带宽 ≈10 Hz（与其设计参数一致）、LQR 更高、
  PID/级联 3.8–4.1 Hz（车辆调参偏无超调牺牲带宽）——各有其理。
  带宽插值精度受频段间距限制（2→5 Hz 跨膝部实测 ~9%）。

### 修复 (Fixed)

- **进阶控制器车辆闭环调参轮（迭代方向 1）**：dob/adrc/mpc/h_inf 默认值
  从对象级整定升级为车辆闭环调参，阶跃 settle 全面进入 v1 档
  （dob 2.5→0.13 s、adrc 2.5→0.10 s、mpc 0.82→0.055 s——**全场最优**、
  h_inf 1.575→0.08 s）。过程再抓三个真问题：
  1. **ADRC 与 DOB 同病**：ESO 把负载折进 f̂ 抵消，叠加齿条力前馈双重
     补偿（调参 cost 12 vs 裸跑 0.3 s settle）——ADRC 同样构造期拒绝
     rack/friction 前馈块。
  2. **ESO 显式欧拉积分的采样脆弱性**：ωo·h ≳0.2 后大角度发散（小角度
     车辆工况被幅值掩盖）——改**精确 ZOH 离散 + Ackermann 极点配置**
     （z = exp(−ωo·h) 三重极点），对偶 φ(A^T) 用错矩阵的 bug 一并修掉。
  3. **观测器/规划器的力矩限必须对齐执行器**：ESO 积分"以为施加的力矩"，
     未限幅指令 vs 已限幅对象 = 纯模型失配——adrc/dob/h_inf 默认
     torque_limit 与 mpc 的 u_max 统一对齐 120 N·m；mpc 盒限变化后
     QP 权重整体失效，已按新盒限重调（首轮 40 N·m 下的解不可迁移）。

- **真实工况验证轮（9 控制器 × 5 工况）抓到并修复四类问题**
  （`scripts/validate_tracking_scenarios.py` 全绿，详见
  `docs/steering_control_guide.md` §3）：
  1. **角执行器默认峰值 40 → 120 N·m**：0.5 g 工况每角回正负载 ≈40 N·m
     恰好压在旧极限上，轮子被负载吹离指令（3° 阶跃 @60 km/h 实测漂到
     −0.34 rad 且 PID/前馈均被同一限幅封死）。按仓库选型模块定尺寸
     （全锁停车 ≈104 N·m/角），零裕度改为有裕度。
  2. **SMC 切换增益 12 → 25 N·m**：滑动可达性条件 |d|<k 被高速直行回正
     负载（≈12 N·m）违反，tanh 饱和后轮子被吹到转向限位（weave@100 实测）。
  3. **DOB 前馈互斥语义**：齿条力/摩擦前馈与扰动观测器对同一扰动双重
     补偿（观测器无法从 u 中区分"抵消负载的力矩"与"负载本身"），轮子
     漂离指令 0.26-0.6 rad——现在构造期明确拒绝这两个前馈块，基底改无
     积分（DC 动作归观测器），估计钳位 60 N·m。
  4. **性能两处**：MPC 凝聚 QP 矩阵预计算（2000 Hz 内环不再做矩阵幂，
     单角 28× 实时）；LQR 构造期 DARE 的 Lyapunov 改倍平方算法
     （5.6 s → 1.1 ms，增益不变）。
  另外 adrc/h_inf 补上前馈栈接口。超包线行为（0.8 g 指令下深滑移回正
  反吹）如实保留为选型维度发现，由偏差通道标记。
...
- **测试并行化（S5）**：dev 依赖新增 `pytest-xdist`，全量套件并行跑通 ——
  ...
  唯一用 socket 的测试让 OS 分配端口。标定辨识两个最重的测试把计算预算
  修剪了约一半（恢复 63→41 s、可复现 37→14 s，容差与断言不变）；
...
- **参数辨识（C3 · 3.1b）**：`sim4wis calibrate fit --reference <台架CSV>
  ...
  台架"变成一个确定性优化问题：5 个中心区转向标量（扭杆刚度、柱阻尼/摩擦、
  齿条库仑摩擦/粘性）经纯 numpy Nelder-Mead（边界归一化坐标、固定初始单纯形、
  评估记忆化）最小二乘拟合，目标函数为逐通道 σ 归一残差 RMS 之和（`steer_hand_angle`
  为指令回显、不进目标）。**两遍拟合**：对齐在第一遍后于拟合结果上重估再拟合一次
  ——手轮角经抓持阻抗随参数微移，单遍对齐会把失配读成几毫秒的伪时钟偏差。
  验收演示：真值 520 N 摩擦 + 3.0 N·m/deg 扭杆的合成台架，从默认值 (260, 2.0)
  出发两参数拟合恢复 519.2 / 2.989（误差 ≤0.4%），全通道残差 rms/σ → 0.001。
  产物：HTML 报告（默认 vs 拟合参数表、前后残差表、三线叠加图）+ JSON 记录
  （参数/边界/评估数/耗时/版本）。护栏不变：**只报告，不回写参数库**（晋级是
  3.1c 的独立评审动作）；测试钉住可复现性与"参数库未被触碰"。
- **残差面板对齐改进（3.1a 顺带）**：对齐通道遍历偏好列表时跳过两侧恒定的通道
  （`driver_steering` 在脚本工况下全程为 0，此前会以 shift=0 带病返回）；互相关
  峰值加抛物线子网格细化，消除 10 ms 网格量化在逐通道残差里留下的几个百分点 σ
  的地板。
- **参数空间余量（C5 + C3c · 判定鲁棒性）**：`sim4wis targets check <set>
  --margins` —— 符合性表的余量是**输出空间**的（实测 vs 限值）；此开关补上
  **参数空间**的回答："参数要错多少，判定才翻"。逐轴（默认 6 个：电机
  峰值/连续转矩/空载转速 + 齿条库仑/粘性 + 扭杆刚度）单轴扰动、以「违反的
  强制条目集合」为签名、二分找交叉点，每步一次完整选型走查 + 判定。翻转条目
  **标注去向**：`→ 达标` 才是合规，`→ 未评估` 是运行超出作动器能力、测量被
  拒绝判定——实测把电机峰值从 8 降到 ~7.2 N·m 时电功率/手力矩两条 FAIL 就以
  `→ 未评估` 离开违反集合（更小的电机让助力更早饱和、驾驶员限扭接管），
  不标注就会读成"换小电机就合规"。`--fit-record <calibrate fit 的 JSON>` 把
  3.1b 的辨识值标到对应轴上并判定其是否落在判定存活区间内——把"参数是估计值"
  变成评审可行动的比较：不覆盖标定偏移的余量就是已经花掉的余量。产物进 CLI
  表格 / `--json` / HTML 报告（新"参数空间余量"节）。已知边界如实声明：单轴
  扰动、沿射线二分，非单调签名（饱和区）给出的是被括住的交叉点而非最近者；
  联合扰动不在承诺范围。

## 0.102.0 — 2026-08-13（P0 正确性修复 · v2 黄金锚点 · 多速率转向 · 摩擦频域分离 · 实验体系统一 · MCP · 标定工作台 · 渲染平滑性验证）

详见 [`docs/v0.102.0_changelog.md`](docs/v0.102.0_changelog.md)。
...
- **0.2 Hz weave 的迟滞环宽对齿条库仑摩擦不单调**：0 N 时 0.99 N·m，500 N 时 0.26，
  900 N 时 1.66。0.2 Hz 下车辆自身侧向动力学滞后贡献了同量级的正交分量，与摩擦项
  部分抵消——两种效应都真实存在。**摩擦要求在 0.02 Hz 慢斜坡上判定**
  （`steering_feel@3`，见 C4），0.2 Hz 上仍只记录不判定，并由测试钉住。
