...
<h3>1.1 为什么需求是一张面而不是一个数</h3>
<p>零侧偏后轮转向律给出的比值 <code>k(v)</code> 由线性二自由度模型闭式解出：
低速趋于 <code>−b/a</code>（本车 {k0:.2f}，几乎 1:1 反相），
高速收敛到 {k120:.2f} 左右（同相）。看上去高速比值不小，
但实际索取的<b>角度</b>是比值乘以前轮转角，而前轮转角在两端相差一个数量级——
120 km/h 做 4 m/s² 只要两三度，泊车掉头则顶在 {front:.0f}° 机械限位上。
两个因子相乘，需求在高速端塌缩、在低速端爆炸。</p>
...
控制律固定为 {law}。每个权限档下的前轮转角<b>单独二分标定</b>到该工况的目标侧向加速度——
限幅会改变转向增益，共用一个角度就会把不同工作点混为一谈。</p>

<h3>1.3 工况</h3>
<div class="grid3">{case_cards}</div>

<h2><span class="n">2</span>需求谱：每个工况到底要多少度</h2>
{fig_map}
{fig_demand}
...
<h2><span class="n">3</span>价值曲线与拐点</h2>
{fig_low}
...
<p>机理在第一篇里已经查清：<code>zero_sideslip_ratio</code> 是常侧偏刚度自行车模型的闭式解，
真实轮胎在几 m/s² 下的等效轴刚度已偏离该常数，解析比因此索取<b>过量</b>的同相后轮转角，
把 β 推过零点变号。权限越大，过冲越充分，|β| 就越大。
<b>这不是「角度多了有害」，是「开环律配上足够权限会暴露自身误差」。</b>
闭环律（横摆反馈）不吃这个亏——第一篇里它在同一硬件上拿到全部九个案例中最低的极限侧偏角。</p></div>
...
<h2><span class="n">4</span>选型建议</h2>
<div class="rec"><table>
...
<h2><span class="n">5</span>局限</h2>
<ul class="tight">
<li>控制律固定为开环速度调度。§3 的「最优权限」现象是<b>该律</b>的性质；
    闭环律的曲线形状会不同，本文未扫描控制律（第一篇扫了）。</li>
...
<li>低速工况在 <b>0–12° 全段不饱和</b>，边际收益近似恒定，
    12° 处泊车直径仍在下降。</li>
<li>配开环解析律时，高速工况的<b>最优权限（{opt_lo:.1f}–{opt_hi:.1f}°）小于需求饱和点
    （{highmax:.2f}°）</b>：超过最优点侧偏角回升。这不是硬件结论而是控制律结论——
    给足权限会把开环律自身的模型误差完整暴露出来。</li>
