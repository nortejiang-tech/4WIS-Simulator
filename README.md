...
四轮独立转向（4-Wheel Independent Steering）工程研究平台。当前版本是 `v0.104.0`，默认车辆标定为智己 LS9，已经形成从实时驾驶、参数建模、实验批跑、KPI 分析、run 回放、安全研究报告到 macOS/Windows 便携版发布的闭环。

项目仍定位为内部预研和工程分析工具，不是经过实车标定或认证的安全结论工具。模型可信度和边界见 [docs/validation_matrix.md](docs/validation_matrix.md)。

> v0.104.0 科学模型评审与交互重构：修正了质心/轴距中点混用、RK4 状态副作用、非线性轮速积分、主销/齿条力链、状态重置与轮位显示；新增手动、脚本和隔离 Agent 的统一结果流。见 [物理评审证据](docs/reports/astra_review_2026-09-08.md) 与 [交互验收记录](docs/reports/interaction_review_2026-09-09.md)。模型尚未完成实车标定，历史版本的手册、研究 HTML、run 和便携包不得与本版结果混用。

## 当前能力

- 三种交互入口：手动驾驶、实时脚本工况、隔离 Agent 会话；标准 OpenAPI/MCP 支持整步控制、幂等重试、完整数据与图表输出。见 [交互与接口使用说明](docs/interaction_guide.md)。
...
<!-- status:begin — generated by scripts/refresh_readme_status.py via pre_release_check; do not edit by hand -->
当前 `v0.104.0` 验证基线（本块由 pre_release_check 在真实跑完门禁后生成，勿手改）：

- 后端：`987 passed`（pytest，并行）
- 前端单测：`54 passed`（vitest）
- smoke：`32/32 通过`
- 黄金实验：`6/6` —— step_steer_60kmh、iso3888_dlc_60kmh、oncentre_weave_100kmh、sw_straight100_rl_stuck_value_baseline、sw_straight100_rl_stuck_value_mitigated、sw_curve60_fl_free_caster_baseline
- 外部参考 benchmark：`2/2`（解析）
- 浏览器 smoke：Playwright Chromium `57 passed`（覆盖清单见下条）
<!-- status:end -->
...
随后验证两个 zip 的当前源码、MCP Agent 入口、数据文件、手册和校验和；当前平台还会从新解压包启动嵌入式后端：

```bash
python scripts/check_portable_package.py --targets macos-arm64 windows-x64 --native-smoke
```

输出在 `dist_portable/`。每个包都提供 Web UI 启动器和 `agent_mcp` 标准接口：在解压目录运行
`agent_mcp.command --print-config`（macOS）或 `agent_mcp.bat --print-config`（Windows）即可得到带当前绝对路径的 MCP 宿主配置。发布到 GitHub Release 时使用 ASCII 文件名。
...
1. 更新版本号、CHANGELOG 和当前交互/物理文档。
2. 构建前端并生成或刷新 `docs/user_manual.html`。
3. 运行 `python scripts/pre_release_check.py`，审阅所有生成报告和 README 状态块。
4. 构建 macOS/Windows 便携包，再运行 `python scripts/check_portable_package.py --native-smoke`。
5. 运行 `python scripts/pre_release_check.py --require-portable-zips`，将当前版本 zip 纳入最终门禁；只有准备声明 v1 外部相关性已就绪时才额外运行 `--strict-v1`。当前独立外部/台架/实车参考数据缺失会使严格 v1 门禁正确地保持未就绪，不阻断一次带明确边界的软件版本发布。
6. 上传 macOS/Windows 便携包、图文手册、交互指南和关键评审报告到 GitHub Release，并回读资产与校验和。
...
- [docs/interaction_guide.md](docs/interaction_guide.md) - 手动、脚本和 Agent 的完整接口、失败语义、单位与全量工件说明。
