...
- 前端生产构建通过且无 Vite chunk warning；默认 `index` chunk 353.90 kB，低于 500 kB 入口预算；懒加载 3D vendor 最大 chunk `vendor-three-core` 666.67 kB，低于 700 kB 3D core 预算；全局 `index.css` 已降到 27.92 kB，车辆几何、负载页和共享 load 图表/控制样式拆为独立 CSS chunks
