...
- 独立 reference 接入：`scripts/scaffold_reference_benchmark.py` 默认把 CarSim/CarMaker、台架、缩比车或实车 benchmark 模板生成到 `validation_data/.incoming/`，不会被正式 checker 误当成证据；补齐真实数据、metrics、notes 后再移入 `validation_data/<benchmark_id>/`
